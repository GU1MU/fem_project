...
MaterialDictReader = Callable[[str], Dict[int, Dict[str, str]]]
...
def _read_tri3_2d_abaqus(
    ...
    *,
    read_materials_as_dict: MaterialDictReader,
    get_float_from_material: MaterialFloatReader,
) -> PlaneMesh2D:
    ...
    if material_path is not None:
        ...
        if material_id not in materials_dict:
            raise KeyError(f"material_id={material_id} 涓嶅湪鏉愭枡琛ㄤ腑锛坢aterial_path={material_path}锛?")
            ...
            if in_node_block:
                ...
                if plane_type is None:
                    ...
                    if pt.startswith("stress"):
                        ...
                        raise ValueError("plane_type 蹇呴』鏄?'stress' 鎴?'strain'")
                ...
                if materials_dict:
                    ...
                    E_val = get_float_from_material(mat_row, ["E", "young", "youngs_modulus"])
                    nu_val = get_float_from_material(mat_row, ["nu", "poisson", "poisson_ratio"])
                    rho_val = get_float_from_material(mat_row, ["rho", "rou", "density"])

                    if E_val is None or nu_val is None:
                        raise KeyError(f"鏉愭枡 {material_id} 缂哄皯 E/nu 淇℃伅锛宺ow={mat_row}")
                    props["E"] = E_val
                    props["nu"] = nu_val
    ...
    if not nodes:
        raise ValueError(f"鏈湪 {inp_path} 涓В鏋愬埌 *Node 鏁版嵁")
    if not elements:
        raise ValueError(f"鏈湪 {inp_path} 涓В鏋愬埌 CPS3/CPE3 鐨?*Element 鏁版嵁")
        ...
        for raw in f:
            ...
            if in_elem and elem_abaqus_type is not None:
                ...
                if materials:
                    ...
                    E = get_float_from_material(row, ["E", "young", "youngs_modulus"])
                    nu = get_float_from_material(row, ["nu", "poisson", "poisson_ratio"])
                    rho = get_float_from_material(row, ["rho", "rou", "density"])
    ...
    if fix_orientation:
        for e in elements:
            ...
            if area < 0.0:
                if len(e.node_ids) != 8:
                    raise ValueError(
                        f"Element {e.id} expected 8 nodes for orientation fix, got {len(e.node_ids)}"
                    )
