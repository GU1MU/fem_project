...
EXAMPLE_PATH = REPO_ROOT / "examples" / "cantilever_beam_tet10.py"
RESULT_PATHS = [
    REPO_ROOT / "results" / "cantilever_beam_tet10_nodal_displacements.csv",
    REPO_ROOT / "results" / "cantilever_beam_tet10_element_stress.csv",
    REPO_ROOT / "results" / "cantilever_beam_tet10_nodal_stress.csv",
    REPO_ROOT / "results" / "cantilever_beam_tet10.vtk",
]
...
class CantileverBeamTet10ExampleTests(unittest.TestCase):
    def test_read_tet10_abaqus_ignores_assembly_reference_nodes(self) -> None:
        with _WorkspaceTempDir() as tmp_path:
            inp_path = tmp_path / "tet10_with_reference_point.inp"
            ...
            self.assertEqual(len(mesh.nodes), 10)
            self.assertEqual(len({node.id for node in mesh.nodes}), 10)
